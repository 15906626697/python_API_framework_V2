...
from Common import myRequestTest
import ddt
...
#logger = MyLogger()
# 获取所有的测试数据
excelfile = dir_config.testcase_dir + "/api_info_2.xlsx"
...
class Test_Api(unittest.TestCase):
    ...
    def test_api(self,case_data):
        ...
        logging.info("接口请求数据为：{0}".format(case_data["data"]))
        logging.info("接口请求cookie为：{0}".format(case_data["cookie"]))
        ...
        res = myRequestTest.myRequest(url = case_data["url"], method = case_data["method"], data = case_data["data"],headers = case_data["cookie"])
        print(res.cookies)
        # logging.info("本次接口请求的状态码为：%d" % res.cookies)
