...
class DoExcel:
    ...
    def get_caseDatas_all(self):
        ...
        for index in range(2,self.sh_case_data.max_row+1):
            ...
            case_data["cookie"] = self.sh_case_data.cell(row=index, column=8).value
            # 比对类型
            case_data["compare_type"] = self.sh_case_data.cell(row=index, column=10).value

            # 获取初始值
            init_datas = self.get_init_datas()

            request_data = self.sh_case_data.cell(row=index, column=7).value
            expected_data = self.sh_case_data.cell(row=index, column=9).value
            # 遍历所有的初始化值的键名。如果请求数据当中，有某一个键名，则直接替换。
            case_data["data"] = self._replace_action(init_datas,request_data)
